...
def load_vipe_data(
    ...
    c2ws_np = np.stack(c2ws_list, axis=0)  # (T, 4, 4)
    ...
    c2ws_T44 = torch.from_numpy(c2ws_np).contiguous()
...
def save_all_points_in_world(c2w_poses, points_in_cam_coord, points_color, save_file_name):
    ...
    o3d.io.write_point_cloud(save_file_name, pcd)
...
if __name__ == "__main__":
    # Example usage
    vipe_root = "/root/autodl-tmp/waymo_datasets_vipe_output/segment-10072231702153043603_5725_000_5745_000_with_camera_labels/front"
    starting_frame = 0
    num_frames = 81
    rgb, depth, mask, w2cs, c2ws, Ks = load_vipe_data(
        vipe_root,
        starting_frame_idx=starting_frame,
        resize_hw=(480, 720),
        crop_hw=(480, 720),
    ...
    print(rgb.shape, depth.shape, mask.shape, w2cs.shape, c2ws.shape, Ks.shape)

    all_cam_coords = []
    N = rgb.shape[0]
    for i in range(N):
        depth_map = depth[i, 0].numpy()
        intrinsic = Ks[i].numpy()
        cam_coords = depth_to_cam_coords_points(depth_map, intrinsic)   # (H, W, 3)
        all_cam_coords.append(cam_coords)
    all_cam_coords = np.array(all_cam_coords)   # (N, H, W, 3)
    save_all_points_in_world(c2ws.numpy(), 
                             all_cam_coords, 
                             rgb.permute(0, 2, 3, 1).numpy(), 
                             "/root/test.ply")
