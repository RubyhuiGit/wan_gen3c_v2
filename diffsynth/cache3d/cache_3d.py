...
class Cache3D_Base:
    def __init__(
        ...
        F = input_shape[format_to_indices.get("F", 0)] if "F" in format_to_indices else 1  # 帧数
        N = input_shape[format_to_indices.get("N", 0)] if "N" in format_to_indices else 1  # 缓存的数量
        V = input_shape[format_to_indices.get("V", 0)] if "V" in format_to_indices else 1  # 视角数量
        ...
        # input_image:   (1, 121, 1, 1, 4, 704, 1280)   (B, F, N, V, C, H, W)
        ...
        self.input_image = self.input_image.to(weight_dtype).to("cpu")   # torch.Size([1, 121, 1, 1, 3, 704, 1280])

        # self.input_mask torch.Size([1, 121, 1, 1, 1, 704, 1280])
        ...
        if input_points is not None:
            ...
            # 从depth + intrinsic + w2c恢复world坐标系下的点云
            self.input_points = (
        ...
        # self.input_depth  torch.Size([121, 1, 704, 1280])
        # self.input_points  torch.Size([1, 121, 1, 1, 704, 1280, 3])
        # self.filter_points_threshold 0.05
        # foreground_masking False
        if self.filter_points_threshold < 1.0 and input_depth is not None:
            ...
            if self.input_mask is None:
                ...
                self.input_mask = self.input_mask * depth_mask.to(self.input_mask.device)    # 输入的mask和depth mask求&
    ...
    def render_cache(self, target_w2cs, target_intrinsics, render_depth=False, start_frame_idx=0):
        ...
        # torch.Size([1, 121, 4, 4]) torch.Size([1, 121, 3, 3]) 0  F_target=121
        # print(target_w2cs.shape, target_intrinsics.shape, start_frame_idx)

        target_w2cs = target_w2cs.reshape(B, F_target, 1, 4, 4).expand(B, F_target, N, 4, 4).reshape(-1, 4, 4)    # torch.Size([121, 4, 4])
        target_intrinsics = (
            target_intrinsics.reshape(B, F_target, 1, 3, 3).expand(B, F_target, N, 3, 3).reshape(-1, 3, 3)        # torch.Size([121, 3, 3])
        )

        # Keep large tensors on CPU; move only per-chunk slices to GPU inside the loop

        # first_images（ batch, frame_num, cache_num合并）=121  视角数量=1   ([121, 1, 3, 704, 1280])
        ...
        # points  torch.Size([121, 1, 704, 1280, 3])
        ...
        # mask torch.Size([121, 1, 1, 704, 1280])
        ...
        if first_images.shape[1] == 1:
            ...
            first_images = first_images.squeeze(1)    # ([121, 3, 704, 1280])
            first_points = first_points.squeeze(1)    # torch.Size([121, 704, 1280, 3])
            first_masks = first_masks.squeeze(1) if first_masks is not None else None  # torch.Size([121, 1, 704, 1280])

            # warp_chun_size永远=2  间隔2
            for i in range(0, first_images.shape[0], warp_chunk_size):
                with torch.no_grad():
                    ...
                    # forward_warp的核心功能是
                    # 通过cache里面换成的点云(来自img_chunk & masks)，计算cache缓存的2帧和要渲染的两帧的光流（由transformation2 & intrinsic2指定）
                    # 然后将cache存放的像素 + masks warp到要渲染的视角下
                    (
                        ...
                        imgs_chunk,            # ([2, 3, 704, 1280])
                        mask1=masks_chunk,     # torch.Size([2, 1, 704, 1280])
                        depth1=None,
                        ...
                        world_points1=pts_chunk,        # torch.Size([2, 704, 1280, 3])
            ...
            raise NotImplementedError     # 多视角，不实现
