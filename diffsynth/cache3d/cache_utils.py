...
try:
    import warp as wp      # pip3 install warp-lang==1.7.2
except ImportError:
...
# 已知条件  frame1   ([2, 3, 704, 1280])
#          mask1     torch.Size([2, 1, 704, 1280])
#          depth1    None
#          transformation1 None    transformation2 要渲染的2帧pose
#          intrinsic1  赋值的要渲染的 intrisic   intrinsic2  要渲染的intrinsic
#          world points     torch.Size([2, 704, 1280, 3])
#          render_depth False foreground_masking False  boundary_mask None
def forward_warp(
    frame1: torch.Tensor,
    ...
    if depth1 is None:
        ...
        if foreground_masking:
            ...
            trans_points1 = project_points(world_points1, transformation2, intrinsic2)     # 将cache里面存放的世界坐标系下的点，投影到要渲染的2帧像素坐标系下
    else:
    ...
    flow12 = trans_coordinates - grid    # 从cache的原来的两帧 -> 要渲染的2帧的光流   (cache存放的世界坐标系的点云，是有序的)
    ...
    # 根据光流，将frame1，mask1 warp到要渲染的两帧上去
