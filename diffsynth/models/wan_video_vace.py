...
class VaceWanModel(torch.nn.Module):
    ...
    def forward(
        ...
        # print(x.shape, vace_context.shape, context.shape, t_mod.shape, freqs.shape)    改vace_in_dim=96
        # torch.Size([1, 20280, 1536]) torch.Size([1, 96, 13, 60, 104]) torch.Size([1, 512, 1536]) torch.Size([1, 6, 1536]) torch.Size([20280, 1, 64])

        c = [self.vace_patch_embedding(u.unsqueeze(0)) for u in vace_context]   # c[0] torch.Size([1, 1536, 13, 30, 52])
        c = [u.flatten(2).transpose(1, 2) for u in c]    # torch.Size([1, 20280, 1536])
        c = torch.cat([
            torch.cat([u, u.new_zeros(1, x.shape[1] - u.size(1), u.size(2))],
                      dim=1) for u in c
        ])    # torch.Size([1, 20280, 1536])
        ...
        for block in self.vace_blocks:
            if use_gradient_checkpointing_offload:
                ...
                c = block(c, x, context, t_mod, freqs)   # 累加拼接 输入输出都是 torch.Size([1, 20280, 1536])
        
        hints = torch.unbind(c)[:-1]       # 每层  (16, 1, 20280, 1536) ->. 1, 20280, 1536 
        return hints
