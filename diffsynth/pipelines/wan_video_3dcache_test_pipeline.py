...
class WanVideo3DCacheTestPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.units = [
            ...
            WanVideoUnit_3DCacheRenderTestPipe(),
    ...
    def vace_from_pretrained(self, state_dict, strict=True):
        self.vace.load_state_dict(state_dict)
        self.vace.eval()
        self.vace.requires_grad_(False)
        print("VACE loaded.")
    ...
    def from_pretrained(
        ...
        # Initialize pipeline
        pipe = WanVideo3DCacheTestPipeline(device=device, torch_dtype=torch_dtype)
        ...
        pipe.vace = VaceWanModel(vace_in_dim=34).to(device).to(torch_dtype)
    ...
    def __call__(
        ...
        inputs_shared = {
            "target_w2c": dataset_info["target_w2c"],        # (N, 4, 4)
            "target_intrinsic": dataset_info["target_intrinsic"],   # (N, 3, 3)
            "cache_frames": dataset_info["cache_frames"],    # (N, 480, 640, 3)
            "cache_w2cs": dataset_info["cache_w2cs"],        # (N, 4, 4)
            "cache_intrinsics": dataset_info["cache_intrinsics"],      # (N, 3, 3)
            "cache_depths": dataset_info["cache_depths"],    # (N, 480, 640)
            "cache_index": dataset_info["cache_index"],      # [0, 20]
            "height": dataset_info["height"],                # 480
            "width": dataset_info["width"],                  # 640
            "num_frames": dataset_info["num_frames"],        # 21
        ...
        for unit in self.units:
            # print(f"Executing {unit.__class__.__name__} ...")
...
class WanVideoUnit_3DCacheRenderTestPipe(PipelineUnit):
    def __init__(self):
        super().__init__(input_params=("cache_frames", "cache_depths", "cache_w2cs", "cache_intrinsics", "target_w2c", "target_intrinsic", 
                                                                                                            "tiled", "tile_size", "tile_stride", "vace_scale"))

    def process(self, pipe: WanVideo3DCacheTestPipeline, 
                      cache_frames, cache_depths, cache_w2cs, cache_intrinsics, target_w2c, target_intrinsic, tiled, tile_size, tile_stride, vace_scale):

        # cache
        cache_frames = torch.from_numpy(cache_frames)            # torch.Size([2, 3, 480, 640])
        cache_depths = torch.from_numpy(cache_depths)            # torch.Size([2, 480, 640])
        cache_w2c = torch.from_numpy(cache_w2cs)                 # torch.Size([2, 4, 4])
        cache_intrinsics = torch.from_numpy(cache_intrinsics)    # torch.Size([2, 3, 3])

        cache_frames = cache_frames.float() / 255.0
        cache_frames = cache_frames.permute(0, 3, 1, 2)
        normalizer = transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
        cache_frames = normalizer(cache_frames)

        cache_frames = cache_frames.unsqueeze(0)
        cache_depths = cache_depths.unsqueeze(0).unsqueeze(2)
        cache_w2c = cache_w2c.unsqueeze(0)
        cache_intrinsics = cache_intrinsics.unsqueeze(0)

        cache = Cache3D_BufferSelector(
            frame_buffer_max=2,
            input_image=cache_frames,           # torch.Size([1, 2, 3, 480, 640])
            input_depth=cache_depths,           # torch.Size([1, 2, 1, 480, 640])
            input_mask=None,
            input_w2c=cache_w2c,                # torch.Size([1, 2, 4, 4])
            input_intrinsics=cache_intrinsics,  # torch.Size([1, 2, 3, 3])
        ...
        # target
        target_w2c = torch.from_numpy(target_w2c).unsqueeze(0)
        target_intrinsic = torch.from_numpy(target_intrinsic).unsqueeze(0)
        ...
        # if True:
        #     self.debug_show(render_imgs, render_masks)
        ...
        for i, (render_pixel, render_mask) in enumerate(zip(mask_pixel_values, masks)):
            ...
            render_mask = render_mask.to(device=pipe.device, dtype=pipe.torch_dtype)
            render_mask = render_mask.permute(0, 2, 1, 3, 4)   # torch.Size([1, 1, 21, 480, 640])
            render_mask = torch.nn.functional.interpolate(render_mask, 
                                    size=((render_mask.shape[2] + 3) // 4,
                                          render_mask.shape[3] // 8, 
                                          render_mask.shape[4] // 8), mode='nearest-exact')  # (1, 1, 6, 60, 80)
            render_view1 = torch.cat((render_pixel_latents, render_mask), dim=1)   # torch.Size([1, 17, 6, 60, 80])
            latent_condition.append(render_view1)

        vace_context = torch.cat(latent_condition, dim=1)     # torch.Size([1, 34, 6, 60, 80])
        return {"vace_context": vace_context, "vace_scale": vace_scale}

    def debug_show(self, render_imgs, render_masks):
        cur_render_image = render_imgs[0].clone().detach()
        cur_render_image = (cur_render_image * 0.5 + 0.5) * 255.0        # B=81, N=2, C=3, H, W
        B, N, C, H, W = cur_render_image.shape

        all_view_img = []
        for i in range(B):
            render_view1_img = cur_render_image[i, 0].permute(1, 2, 0).cpu().numpy().astype(np.uint8)
            render_view2_img = cur_render_image[i, 1].permute(1, 2, 0).cpu().numpy().astype(np.uint8)
            view_img = np.concatenate([render_view1_img, render_view2_img], axis=1)
            all_view_img.append(view_img)
        all_img_show = np.stack(all_view_img, axis=0)
        import imageio
        imageio.mimsave(f"/root/origin_render.gif", all_img_show, fps=10)
        ...
        imageio.mimsave(f"/root/render_mask.gif", view_mask_rgb, fps=10)
...
class WanVideoUnit_ShapeChecker(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTestPipeline, height, width, num_frames):
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTestPipeline, height, width, num_frames, seed, rand_device, vace_reference_image):
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTestPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image):
        ...
        if pipe.scheduler.training:
            return {"latents": noise, "input_latents": input_latents}     # torch.Size([1, 16, 6, 60, 80]) torch.Size([1, 16, 6, 60, 80])
        else:
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTestPipeline, prompt, positive) -> dict:
...
class WanVideoUnit_ImageEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTestPipeline, input_image, end_image, num_frames, height, width, tiled, tile_size, tile_stride):
...
class WanVideoUnit_ImageEmbedderCLIP(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTestPipeline, input_image, end_image, height, width):
...
class WanVideoUnit_ImageEmbedderFused(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTestPipeline, input_image, latents, height, width, tiled, tile_size, tile_stride):
...
class WanVideoUnit_FunControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTestPipeline, control_video, num_frames, height, width, tiled, tile_size, tile_stride, clip_feature, y, latents):
...
class WanVideoUnit_FunReference(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTestPipeline, reference_image, height, width):
...
class WanVideoUnit_FunCameraControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTestPipeline, height, width, num_frames, camera_control_direction, camera_control_speed, camera_control_origin, latents, input_image, tiled, tile_size, tile_stride):
...
class WanVideoUnit_SpeedControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTestPipeline, motion_bucket_id):
...
#     def process(
#         self,
#         pipe: WanVideo3DCacheTestPipeline,
...
#             vace_video_latents = torch.concat((inactive, reactive), dim=1)    # torch.Size([1, 16+16, 13, 60, 104])
            
#             # torch.Size([1, 64, 13, 60, 104])
...
#             vace_context = torch.concat((vace_video_latents, vace_mask_latents), dim=1)    # torch.Size([1, 64+32, 13, 60, 104])
#             return {"vace_context": vace_context, "vace_scale": vace_scale}
#         else:
...
class WanVideoUnit_TeaCache(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTestPipeline, num_inference_steps, tea_cache_l1_thresh, tea_cache_model_id):
...
class WanVideoUnit_CfgMerger(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTestPipeline, inputs_shared, inputs_posi, inputs_nega):
...
class WanVideoUnit_S2V(PipelineUnit):
    ...
    def process_audio(self, pipe: WanVideo3DCacheTestPipeline, input_audio, audio_sample_rate, num_frames, fps=16, audio_embeds=None, return_all=False):
    ...
    def process_motion_latents(self, pipe: WanVideo3DCacheTestPipeline, height, width, tiled, tile_size, tile_stride, motion_video=None):
        pipe.load_models_to_device(["vae"])
        motion_frames = 73
        kwargs = {}
    ...
    def process_pose_cond(self, pipe: WanVideo3DCacheTestPipeline, s2v_pose_video, num_frames, height, width, tiled, tile_size, tile_stride, s2v_pose_latents=None, num_repeats=1, return_all=False):
    ...
    @staticmethod
    def pre_calculate_audio_pose(pipe: WanVideo3DCacheTestPipeline, input_audio=None, audio_sample_rate=16000, s2v_pose_video=None, num_frames=81, height=448, width=832, fps=16, tiled=True, tile_size=(30, 52), tile_stride=(15, 26)):
...
class WanVideoPostUnit_S2V(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTestPipeline, latents, motion_latents, drop_motion_frames):
