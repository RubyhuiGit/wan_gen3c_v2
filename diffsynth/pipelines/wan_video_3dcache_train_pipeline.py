...
class WanVideo3DCacheTrainPipeline(BasePipeline):
    ...
    def from_pretrained(
        ...
        # Initialize pipeline
        pipe = WanVideo3DCacheTrainPipeline(device=device, torch_dtype=torch_dtype)
...
class WanVideoUnit_3DCacheRender(PipelineUnit):
    def __init__(self):
        super().__init__(input_params=("input_video", "input_w2c", "input_intrinsics", "input_depth", "cache_index", "tiled", "tile_size", "tile_stride", "vace_scale"))

    def process(self, pipe: WanVideo3DCacheTrainPipeline, 
                      input_video, 
                      input_w2c, 
                      input_intrinsics, 
                      input_depth, 
        ...
        input_w2c = torch.from_numpy(input_w2c)
        input_intrinsics = torch.from_numpy(input_intrinsics)
        input_depth = torch.from_numpy(input_depth)
        cache_index = torch.tensor(cache_index, dtype=torch.long)

        # cache
        cache_frames = torch.index_select(input_video, 0, cache_index)            # torch.Size([2, 3, 480, 640])
        cache_depths = torch.index_select(input_depth, 0, cache_index)            # torch.Size([2, 480, 640])
        cache_w2c = torch.index_select(input_w2c, 0, cache_index)                 # torch.Size([2, 4, 4])
        cache_intrinsics = torch.index_select(input_intrinsics,0, cache_index)    # torch.Size([2, 3, 3])
        ...
        # target
        target_w2c = input_w2c.unsqueeze(0)
        target_intrinsics = input_intrinsics.unsqueeze(0)
        ...
        render_imgs, render_masks = cache.render_cache(
            ...
            target_intrinsics,
            start_frame_idx=0,
        )    # torch.Size([1, N, 2, 3, 480, 640]) torch.Size([1, N, 2, 1, 480, 640])  -1, 1

        # if True:
        #     self.debug_show(input_video, render_imgs, render_masks)
    ...
    def debug_show(self, input_video, render_imgs, render_masks):
        cur_imgs = input_video.numpy().astype(np.uint8)       # 81, H, W, 3
        ...
        for i in range(B):
            ...
            view_img = np.concatenate([origin_img, render_view1_img, render_view2_img], axis=1)
...
class WanVideoUnit_ShapeChecker(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTrainPipeline, height, width, num_frames):
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTrainPipeline, height, width, num_frames, seed, rand_device, vace_reference_image):
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTrainPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image):
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTrainPipeline, prompt, positive) -> dict:
...
class WanVideoUnit_ImageEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTrainPipeline, input_image, end_image, num_frames, height, width, tiled, tile_size, tile_stride):
...
class WanVideoUnit_ImageEmbedderCLIP(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTrainPipeline, input_image, end_image, height, width):
...
class WanVideoUnit_ImageEmbedderFused(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTrainPipeline, input_image, latents, height, width, tiled, tile_size, tile_stride):
...
class WanVideoUnit_FunControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTrainPipeline, control_video, num_frames, height, width, tiled, tile_size, tile_stride, clip_feature, y, latents):
...
class WanVideoUnit_FunReference(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTrainPipeline, reference_image, height, width):
...
class WanVideoUnit_FunCameraControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTrainPipeline, height, width, num_frames, camera_control_direction, camera_control_speed, camera_control_origin, latents, input_image, tiled, tile_size, tile_stride):
...
class WanVideoUnit_SpeedControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTrainPipeline, motion_bucket_id):
...
#     def process(
#         self,
#         pipe: WanVideo3DCacheTrainPipeline,
...
class WanVideoUnit_TeaCache(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTrainPipeline, num_inference_steps, tea_cache_l1_thresh, tea_cache_model_id):
...
class WanVideoUnit_CfgMerger(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTrainPipeline, inputs_shared, inputs_posi, inputs_nega):
...
class WanVideoUnit_S2V(PipelineUnit):
    ...
    def process_audio(self, pipe: WanVideo3DCacheTrainPipeline, input_audio, audio_sample_rate, num_frames, fps=16, audio_embeds=None, return_all=False):
    ...
    def process_motion_latents(self, pipe: WanVideo3DCacheTrainPipeline, height, width, tiled, tile_size, tile_stride, motion_video=None):
        pipe.load_models_to_device(["vae"])
        motion_frames = 73
        kwargs = {}
    ...
    def process_pose_cond(self, pipe: WanVideo3DCacheTrainPipeline, s2v_pose_video, num_frames, height, width, tiled, tile_size, tile_stride, s2v_pose_latents=None, num_repeats=1, return_all=False):
    ...
    @staticmethod
    def pre_calculate_audio_pose(pipe: WanVideo3DCacheTrainPipeline, input_audio=None, audio_sample_rate=16000, s2v_pose_video=None, num_frames=81, height=448, width=832, fps=16, tiled=True, tile_size=(30, 52), tile_stride=(15, 26)):
...
class WanVideoPostUnit_S2V(PipelineUnit):
    ...
    def process(self, pipe: WanVideo3DCacheTrainPipeline, latents, motion_latents, drop_motion_frames):
